...
class SyraUser(AbstractUser):
    ...
    def is_doctor(self):
        ...
        return self.profile_role == self.ProfileRole.DOCTOR or self.is_staff

    @property
    def is_engineer(self):
        """Check if user is an engineer/worker."""
        return self.profile_role == self.ProfileRole.ENGINEER or self.is_staff

    @property
    def is_profile_admin(self):
        """Check if user has admin access to profiles."""
        return self.profile_role == self.ProfileRole.ADMIN or self.is_staff

    @property
    def can_view_full_profile(self):
        """Check if user can view full medical profile."""
        return (
            self.profile_role in [self.ProfileRole.DOCTOR, self.ProfileRole.ADMIN]
            or self.is_staff
        )
    ...
    def can_manage_products(self):
        ...
        return (
            self.store_role
            in [self.StoreRole.PRODUCT_MANAGER, self.StoreRole.STORE_ADMIN]
            or self.is_staff
        )
    ...
    def can_manage_prices(self):
        ...
        return (
            self.store_role
            in [self.StoreRole.PRICE_MANAGER, self.StoreRole.STORE_ADMIN]
            or self.is_staff
        )
    ...
    def can_manage_orders(self):
        ...
        return (
            self.store_role in [self.StoreRole.STORE_ADMIN, self.StoreRole.STORE_VIEWER]
            or self.is_staff
        )
